(function () {
    'use strict';

    /**
     * ANIMATED MOVIES MASTER COLLECTION
     * Feature Animation Films (Excludes TV Series, JP, KO, ZH, RU, BE Animation)
     */

    var EXCLUDED_LANGS = ['ru', 'be', 'ja', 'ko', 'zh'];

    var ANIMATED_MOVIES_CONFIG = {
        title: 'Мультфільми',
        icon: '<svg viewBox="0 0 24 24" fill="#FFC107" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>',
        categories: [
            // --- ГОЛОВНІ ТРЕНДИ ---
            { 
                "title": "🔥 Тренди (За останній рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },

            // --- ЖАНРИ ЗА ОСТАННІЙ РІК ---
            { 
                "title": "🤠 Пригоди та Екшн (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,12", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "🧙‍♂️ Фентезі та Казки (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,14", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "🚀 Фантастика (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,878", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "😂 Комедії (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,35", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "👨‍👩‍👧 Сімейні мультфільми (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,10751", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "💖 Драматичні та Душевні (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,18", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "10",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "🔎 Детективи та Містика (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,9648", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "5",
                    "sort_by": "popularity.desc" 
                } 
            },
            { 
                "title": "🎼 Музичні та Мюзикли (За рік)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16,10402", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "{one_year_ago}", 
                    "primary_release_date.lte": "{current_date}",
                    "vote_count.gte": "5",
                    "sort_by": "popularity.desc" 
                } 
            },

            // --- НАЙКРАЩІ ЗА ДЕСЯТИЛІТТЯ ---
            { 
                "title": "⚡ Сучасний Період 2020-х (Найкраще)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "2020-01-01", 
                    "vote_average.gte": "7.0",
                    "vote_count.gte": "100",
                    "sort_by": "vote_average.desc" 
                } 
            },
            { 
                "title": "💎 Ера 2010-х (Найкраще)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "2010-01-01", 
                    "primary_release_date.lte": "2019-12-31", 
                    "vote_average.gte": "7.5",
                    "vote_count.gte": "300",
                    "sort_by": "vote_average.desc" 
                } 
            },
            { 
                "title": "💿 Культові 2000-ні (Найкраще)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "2000-01-01", 
                    "primary_release_date.lte": "2009-12-31", 
                    "vote_average.gte": "7.5",
                    "vote_count.gte": "300",
                    "sort_by": "vote_average.desc" 
                } 
            },
            { 
                "title": "📼 Золота ера 1990-х (Найкраще)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.gte": "1990-01-01", 
                    "primary_release_date.lte": "1999-12-31", 
                    "vote_average.gte": "7.5",
                    "vote_count.gte": "200",
                    "sort_by": "vote_average.desc" 
                } 
            },
            { 
                "title": "🏛️ Класика Анімації (до 1990 року)", 
                "url": "discover/movie", 
                "params": { 
                    "with_genres": "16", 
                    "without_original_language": EXCLUDED_LANGS.join(','), 
                    "primary_release_date.lte": "1989-12-31", 
                    "vote_average.gte": "7.0",
                    "vote_count.gte": "100",
                    "sort_by": "vote_average.desc" 
                } 
            }
        ]
    };

    function resolveParamValue(val) {
        var d = new Date();
        if (val === '{current_date}') {
            return [d.getFullYear(), ('0' + (d.getMonth() + 1)).slice(-2), ('0' + d.getDate()).slice(-2)].join('-');
        }
        if (val === '{one_year_ago}') {
            d.setFullYear(d.getFullYear() - 1);
            return [d.getFullYear(), ('0' + (d.getMonth() + 1)).slice(-2), ('0' + d.getDate()).slice(-2)].join('-');
        }
        return val;
    }

    function fetchWithFallback(urlUk, callback) {
        var network = new Lampa.Reguest();

        network.silent(urlUk, function (jsonUk) {
            if (!jsonUk || !jsonUk.results || !jsonUk.results.length) {
                return callback(jsonUk);
            }

            // Жорсткий JS-фільтр: відсіюємо картки без постерів та небажані мови
            jsonUk.results = jsonUk.results.filter(function (item) {
                if (!item.poster_path) return false;
                if (item.original_language && EXCLUDED_LANGS.indexOf(item.original_language) !== -1) return false;
                return true;
            });

            var needEnglish = jsonUk.results.some(function (item) {
                var title = item.title || item.name || '';
                return !title;
            });

            if (needEnglish) {
                var urlEn = urlUk.replace('language=uk', 'language=en');
                network.silent(urlEn, function (jsonEn) {
                    if (jsonEn && jsonEn.results) {
                        jsonUk.results.forEach(function (item, idx) {
                            var title = item.title || item.name || '';
                            if (!title && jsonEn.results[idx]) {
                                var enTitle = jsonEn.results[idx].title || jsonEn.results[idx].name;
                                if (enTitle) {
                                    if (item.title) item.title = enTitle;
                                    if (item.name) item.name = enTitle;
                                }
                            }
                        });
                    }
                    callback(jsonUk);
                }, function () {
                    callback(jsonUk);
                });
            } else {
                callback(jsonUk);
            }
        }, function () {
            callback(null);
        });
    }

    function AnimatedMoviesMain(object) {
        var comp = new Lampa.InteractionMain(object);

        comp.create = function () {
            var _this = this;
            this.activity.loader(true);
            var categories = ANIMATED_MOVIES_CONFIG.categories;
            var status = new Lampa.Status(categories.length);

            status.onComplite = function () {
                var fulldata = [];
                Object.keys(status.data).sort(function (a, b) { return a - b; }).forEach(function (key) {
                    var data = status.data[key];
                    if (data && data.results && data.results.length) {
                        var cat = categories[parseInt(key)];
                        Lampa.Utils.extendItemsParams(data.results, { style: { name: 'wide' } });
                        fulldata.push({
                            title: cat.title,
                            results: data.results,
                            url: cat.url,
                            params: cat.params
                        });
                    }
                });

                if (fulldata.length) {
                    _this.build(fulldata);
                    _this.activity.loader(false);
                } else {
                    _this.empty();
                }
            };

            categories.forEach(function (cat, index) {
                var params = [];
                params.push('api_key=' + Lampa.TMDB.key());
                params.push('language=' + Lampa.Storage.get('language', 'uk'));

                if (cat.params) {
                    for (var key in cat.params) {
                        var val = cat.params[key];
                        val = resolveParamValue(val);
                        params.push(key + '=' + val);
                    }
                }

                var url = Lampa.TMDB.api(cat.url + '?' + params.join('&'));

                fetchWithFallback(url, function (json) {
                    if (json) status.append(index.toString(), json);
                    else status.error();
                });
            });

            return this.render();
        };

        comp.onMore = function (data) {
            Lampa.Activity.push({
                url: data.url,
                params: data.params,
                title: data.title,
                component: 'animated_movies_view',
                page: 1
            });
        };

        return comp;
    }

    function AnimatedMoviesView(object) {
        var comp = new Lampa.InteractionCategory(object);

        function buildUrl(page) {
            var params = [];
            params.push('api_key=' + Lampa.TMDB.key());
            params.push('language=' + Lampa.Storage.get('language', 'uk'));
            params.push('page=' + page);

            if (object.params) {
                for (var key in object.params) {
                    var val = object.params[key];
                    val = resolveParamValue(val);
                    params.push(key + '=' + val);
                }
            }
            return Lampa.TMDB.api(object.url + '?' + params.join('&'));
        }

        comp.create = function () {
            var _this = this;
            fetchWithFallback(buildUrl(1), function (json) {
                if (json) _this.build(json);
                else _this.empty();
            });
        };

        comp.nextPageReuest = function (object, resolve, reject) {
            fetchWithFallback(buildUrl(object.page), function (json) {
                if (json) resolve(json);
                else reject();
            });
        };

        return comp;
    }

    function startPlugin() {
        if (window.plugin_animated_movies_ready) return;
        window.plugin_animated_movies_ready = true;

        Lampa.Component.add('animated_movies_main', AnimatedMoviesMain);
        Lampa.Component.add('animated_movies_view', AnimatedMoviesView);

        function addMenuButton() {
            var menu = $('.menu .menu__list').eq(0);
            if (!menu.length || menu.find('.menu__item[data-action="animated_movies_master"]').length) return;

            var btn = $(`<li class="menu__item selector" data-action="animated_movies_master">
                <div class="menu__ico">${ANIMATED_MOVIES_CONFIG.icon}</div>
                <div class="menu__text">${ANIMATED_MOVIES_CONFIG.title}</div>
            </li>`);

            btn.on('hover:enter', function () {
                Lampa.Activity.push({
                    title: ANIMATED_MOVIES_CONFIG.title,
                    component: 'animated_movies_main',
                    page: 1
                });
            });

            menu.append(btn);
        }

        if (window.appready) {
            addMenuButton();
        } else {
            Lampa.Listener.follow('app', function (e) {
                if (e.type == 'ready') addMenuButton();
            });
        }

        setInterval(function () {
            if (window.appready && $('.menu .menu__list').eq(0).length) {
                addMenuButton();
            }
        }, 3000);
    }

    if (!window.plugin_animated_movies_ready) startPlugin();
})();
